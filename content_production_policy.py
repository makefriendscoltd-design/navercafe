...
# Both channels retell a talk long enough to hold five copyable steps. Nothing
...
def validate_longform_source(measurement: dict[str, Any]) -> dict[str, Any]:
    """Reject a source that is a Short rather than the longform talk both channels retell."""
    ...
    if seconds <= 0 or width <= 0 or height <= 0:
        raise ProductionPolicyError("원본 길이·해상도를 측정하지 못했습니다.")
    if height > width:
        raise ProductionPolicyError(
            f"원본이 세로 영상(쇼츠)입니다: {width}x{height}")
    if seconds < MIN_LONGFORM_SOURCE_SECONDS:
        raise ProductionPolicyError(
            f"원본이 {seconds}초로 롱폼 기준 {MIN_LONGFORM_SOURCE_SECONDS}초에 못 미칩니다.")
    return {"seconds": seconds, "width": width, "height": height,
            "minimum_seconds": MIN_LONGFORM_SOURCE_SECONDS}

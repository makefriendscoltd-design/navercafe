...
"""
        with genai.Client(api_key=auto.GEMINI_API_KEY) as client:
            resp = client.models.generate_content(
                model=os.environ.get('KEYWORD_MODEL', 'gemini-2.5-flash'),
                contents=prompt,
            )
...
- 첫 문장에서 독자가 얻는 결과나 피할 문제를 바로 보여줄 것
- 숫자는 글 전체의 핵심 단계 수·방법 수처럼 제목의 구조를 설명할 때만 사용
- 예시 차트 입력값, 임시 가격, 토큰 절감률, 벤치마크 수치를 제목에 끌어올리지 말 것
...
def _title_via_gemini(manuscript):
    ...
    try:
        ...
        with genai.Client(api_key=auto.GEMINI_API_KEY) as client:
            resp = client.models.generate_content(
                model=os.environ.get('TITLE_MODEL', 'gemini-2.5-flash'),

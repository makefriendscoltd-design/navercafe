...
let p=null;
try{
 p=await openTab(`${payload.list_url}${payload.list_url.includes('?')?'&':'?'}row_verify=${Date.now()}`);
 await sleep(9000);
 let title=null;
 for(let page=1;page<=80;page++){
  title=await p.evaluate(id=>{const row=[...document.querySelectorAll('ytcp-video-row')]
    .find(e=>e.video?.videoId===id);return row?((row.video?.title||'').trim()):null;},payload.provider_id);
  if(title!==null)break;
 ...
 if(title===null)emit({status:'blocked',error:'direct-list-row-missing'});
 else emit({status:'pass',list_row_title:title});
}catch(error){emit({status:'blocked',error:String(error?.message||error)});}
...
class AsideHeadlessU0Provider:
    ...
    def direct_requery(
        ...
        listed = self._run(
            LIST_ROW_JS,
            {"provider_id": provider_id, "list_url": STUDIO_SHORTS_URL},
            cwd=manifest.video.parent,
            timeout=300,
        )
        if listed.get("status") != "pass":
            raise AdapterEvidenceError(
                f"Studio list row read failed: {listed.get('error') or listed.get('status')}"
            )
        raw = {**raw, "list_row_title": listed.get("list_row_title")}

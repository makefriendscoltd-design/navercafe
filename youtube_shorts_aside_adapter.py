...
if(await editDraft.count()===1&&await editDraft.isVisible()){await editDraft.click();const end=Date.now()+15000;while(Date.now()<end){const dialogs=p.locator('ytcp-uploads-dialog');if(await dialogs.count()===1&&await dialogs.locator('#title-textarea #textbox').isVisible()){root=dialogs;flow='upload';break;}await sleep(200);}if(flow!=='upload')throw new Error('draft-wizard-not-ready');}

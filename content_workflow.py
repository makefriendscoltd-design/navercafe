...
def prepare_cafe(manifest_path: Path, candidate: Path) -> dict:
    ...
    if count_sections(cleaned) != 5:
        ...
        headings = [line.strip() for line in lines[1:] if 4 <= len(line.strip()) <= 45
    ...
    (candidate / '11_local_validation.json').write_text(json.dumps({
        ...
        'checks': {'original_body_preserved': True, 'five_quotes': True, 'five_images': True},
...
def main(argv=None) -> int:
    ...
    if args.command == 'audit-cafe':
        ...
        result = prepare_cafe(args.manifest, args.candidate)
    else:
        ...
        provider = manifest_path.parent / 'notebooklm/notebooklm-provider-evidence.json'
        ...
        try:
            deck = make_card_deck(clean_cafe_answer(answer.read_text(encoding='utf-8')), manifest['title'], content_lineage=origin)
